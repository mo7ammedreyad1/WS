	...
	async fetch(request: Request, env: envData, ctx: ExecutionContext): Promise<Response> {
		...
		// Example: POST /api/send-message { userBot: "bot01", phone: "1234567890", message: "Hello World", adminPassword: "123456" }
			...
			try {
				...
				// IMPORTANT
				// It is necessary to have already stored in KV the auth_info_baileys/creds.json of the sender's WhatsApp account
				// IMPORTANT
				...
				const sockAndSuccessSend = await apiSendMessage(userBot, phone, message, env.KV_whatsappCloudflareWorkers)
		...
		// Example: GET /api/get-all-user-bot
			...
			// KV already returns metadata alongside each key in list(), so no extra per-key lookup (like R2's head()) is needed
			const allUserBotList = await env.KV_whatsappCloudflareWorkers.list({ prefix: `${prefixUserBot}/` })

			const objects: { userBot: string; name: string; phone: string; path: string; created: string; }[] = []
			for (const key of allUserBotList.keys) {
				const metadata = key.metadata as { userBot?: string; name?: string; phone?: string; path?: string; created?: string } | undefined

				objects.push({
					userBot: metadata?.userBot || key?.name?.split("/")?.slice(-2, -1)?.[0] || 'not found',
					name: metadata?.name || 'not found',
					phone: metadata?.phone || 'not found',
					path: metadata?.path || 'not found',
					created: metadata?.created || 'not found'
				})
			}
...
async function apiRegisterWhatsapp(userBot: string, kvFileStorage: KVNamespace): Promise<{sock: ReturnType<typeof makeWASocket>, link: string} | false> {
	try {
		...
		const startSock = async() => {
			const { state, saveCreds } = await useMultiFileAuthState(userBot, kvFileStorage)
...
async function apiSendMessage(userBot: string, phone: string, message: string, kvFileStorage: KVNamespace): Promise<{sock: ReturnType<typeof makeWASocket>, successSend: boolean} | false> {
	try {
		const logger = P({ timestamp: () => `,"time":"${new Date().toJSON()}"` })
		logger.level = 'silent'
		...
		const startSock = async() => {
			const { state, saveCreds } = await useMultiFileAuthState(userBot, kvFileStorage) //CF
			const { version, isLatest } = await fetchLatestBaileysVersion()
			if (logForDevelopment) console.log(`WARNING [startSock = async() => {...] Using WA v${version.join('.')}, isLatest: ${isLatest}`)
